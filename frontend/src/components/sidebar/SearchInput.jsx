...
const SearchInput = () => {
  // "SearchInput" isminde bir fonksiyonel bileşen tanımlar.
  return (
    // Bileşenin render edeceği JSX yapısını başlatır.
    <form className="flex items-center gap-2">
      {" "}
      {/* Flexbox ile yatayda hizalanmış ve aralarındaki boşluğu 2 birim yapan form. */}
      <input
      ...
      <button type="submit" className="btn btn-circle bg-sky-500 text-white">
        {" "}
        {/* Arama butonunu oluşturur. */}
        <IoSearchSharp className="w-6 h-6 outline-none" />{" "}
        {/* Arama simgesi (büyüklüğü 6x6) ve kenar çizgisi yok. */}
      </button>
    </form>
  );
};
